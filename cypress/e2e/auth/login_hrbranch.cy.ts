//cypress/e2e/auth/login_hrbranch.cy.ts
describe("Pengujian Halaman Login HR Branch", () => {
  beforeEach(() => {
  ...
  context("SKENARIO 2: Percobaan Gagal - Input Tidak Valid", () => {
    ...
    it("Harus menampilkan notifikasi alert untuk password yang salah", () => {
      cy.get('input[id="employeeId"]').type(Cypress.env("TEST_HRBRANCH_ID"));
    ...
    it("Harus menampilkan notifikasi alert untuk NIK yang tidak ada", () => {
      ...
      cy.get('input[id="password"]').type(
        Cypress.env("TEST_HRBRANCH_PASSWORD")
      );
      cy.get('button[type="submit"]').click();
  ...
  // ------------------------------------------
  context("SKENARIO 4: Percobaan Berhasil Login HR Branch", () => {
    it("Harus berhasil login dengan kredensial HR Branch yang valid", () => {
      const hrNik = Cypress.env("TEST_HRBRANCH_ID");
      const hrPassword = Cypress.env("TEST_HRBRANCH_PASSWORD");

      if (!hrNik || !hrPassword) {
        throw new Error(
          "Variabel TEST_HRBRANCH_ID dan TEST_HRBRANCH_PASSWORD tidak diatur di cypress.env.json"
        );
      }

      cy.get('input[id="employeeId"]').type(hrNik);
      cy.get('input[id="password"]').type(hrPassword);
      cy.get('button[type="submit"]').click();

      // Verifikasi redirect ke halaman employee dashboard (berdasarkan middleware.ts)
      cy.url({ timeout: 10000 }).should("include", "/admin");

      // Verifikasi elemen di dashboard employee
